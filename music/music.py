...
class Music(commands.Cog, discordSuperUtils.CogManager.Cog):
    ...
    async def on_music_error(self, ctx, error):
        errors = {
            ...
            discordSuperUtils.NotConnected: f"Izumi not connected to a voice channel!",
    ...
    async def on_play(self, ctx, player):  # This returns a player object
        ...
        requester = (
            player.requester.mention if player.requester else "Autoplay"
        )
    ...
    async def disconnect(self, ctx):
        if await self.MusicManager.leave(ctx):
            msg = await ctx.send(embed=discord.Embed(title="disconnecting"))
            await asyncio.sleep(1)
            msg.edit(embed=discord.Embed(title="Disconnected."))
    ...
    async def lyrics(self, ctx, *, query=None):
        if response := await self.MusicManager.lyrics(ctx, query):
            ...
            for i, split in enumerate(splitted):
                if (
                    len(splitted) <= i + 1
                    or len(current) + len(splitted[i + 1]) > 1024
                ):
                    res.append(current)
                    current = ""
                    continue
    ...
    async def now_playing(self, ctx):
        if player := await self.MusicManager.now_playing(ctx):
            ...
            thumbnail = player.data["videoDetails"]["thumbnail"]["thumbnails"][
                -1
            ]["url"]
            title = player.title
            url = player.url
    ...
    async def queueloop(self, ctx):
        ...
        if is_loop is not None:
            await ctx.send(
                f"Queue Looping {'Enabled' if is_loop else 'Disabled'}"
            )
    ...
    async def skip(self, ctx, index: int = None):
        if queue := (await self.MusicManager.get_queue(ctx)):
            ...
            # Checking if queue is empty and autoplay is disabled
                ...
                if (
                    voter
                    == (await self.MusicManager.now_playing(ctx)).requester
                ):
                ...
                # Voting
                    ...
                    if total_votes >= 3:
                        skipped_player = await self.MusicManager.skip(
                            ctx, index
                        )
    ...
    @playlists.command(name="play")
    @ensure_voice_state()
    async def playlist_play(
        self, ctx, index: int, user: discord.member = None
    ):
        ...
        playlists = await self.MusicManager.get_user_playlists(
            user, partial=True
        )
        ...
        user_playlist = await self.MusicManager.get_playlist(
            ctx.author, playlist.id
        )
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            ...
            await ctx.send(
                f"Izumi is already in a voice channel <#{ctx.voice_client.channel.id}>"
            )
