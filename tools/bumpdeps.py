...
def main() -> int:
    ...
    primary_reqs_cfg = setuptools.config.read_configuration(
        str(REQUIREMENTS_INI_PTH)
    )
    ...
    for extra, extra_primary_deps in primary_reqs_cfg["options"][
        "extras_require"
    ].items():
        print(extra, "=")
...
def get_all_reqs(
    ...
    reqs_dict = {
        r.name.lower(): r
        for r in map(packaging.requirements.Requirement, primary_deps)
    }
    with tempfile.TemporaryDirectory() as tmpdir:
        venv.create(
            tmpdir, system_site_packages=False, clear=True, with_pip=True
        )
        ...
        sp.run(
            (pip_exe_pth, *PIP_INSTALL_ARGS, "pip"),
            stdout=sp.DEVNULL,
            check=True,
        )
        ...
        ret = []
        for req_obj in map(
            packaging.requirements.Requirement, proc.stdout.strip().split("\n")
        ):
