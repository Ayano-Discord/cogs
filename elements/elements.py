...
class Elements(commands.Cog):
    ...
    async def element(
        ...
        `measurement` can be any of the Elements data listed [here](https://mendeleev.readthedocs.io/en/stable/data.html#electronegativities)
        """
        if not measurement:
    ...
    async def element_embed(self, element: ELEMENTS) -> discord.Embed:
        ...
        attributes = {
            ...
            "lattice_structure": (
                "Crystal Lattice",
                self.get_lattice_string(element),
            ),
        }
