...
# Music Imports
from .constants import emoji, GOOD_EXTENSIONS, IMGUR_LINKS, REDDIT_BASEURL
...
class Core(commands.Cog):
    ...
    async def _get_imgs(self, ctx: commands.Context, sub: str = None):
        ...
        try:
            async with self.session.get(
                REDDIT_BASEURL.format(choice(sub))
            ) as reddit:
    ...
    async def _api_errors_msg(
        self, ctx: commands.Context, error_code: int = None
    ):
    ...
    async def _make_embed(self, ctx: commands.Context, sub: str, name: str):
        ...
        if url.endswith(GOOD_EXTENSIONS):
            em = await self._embed(
                color=0x891193,
                title=(_("Here is {name} image ...") + " \N{EYES}").format(
                    name=name
                ),
                description=bold(
    ...
    async def _make_embed_other(
        ...
        try:
            data = await asyncio.wait_for(
                self._get_others_imgs(ctx, url=url), 3
            )
        except asyncio.TimeoutError:
            await ctx.send(
                "Failed to get an image. Please try again later. (Timeout error)"
    ...
    async def _send_msg(
        self, ctx: commands.Context, name: str, sub: str = None
    ):
