...
DEFAULT_OFFLINE_EMOJI = "<a:dnd:880327995573796905>"
DEFAULT_ONLINE_EMOJI = "<a:online:880327962019377153>"
...
class Otherbot(commands.Cog):
        ...
        if requester == "discord":
            ...
            for guild_id, guild_data in data.items():
                if user_id in guild_data.get("watching", []):
                    ...
                    await self.config.guild_from_id(guild_id).bypass.set(
                        list(bypass)
                    )
    ...
    def __init__(self, bot: Red):
        ...
        self.config = Config.get_conf(
            self, 2730321001, force_registration=True
        )
    ...
    async def get_watching(
        self, watch_list: list, watch_type: str, guild: int
    ):
        data = []
        ...
        if not ctx.invoked_subcommand:
            ...
            if ctx.channel.permissions_for(ctx.me).embed_links:
                em = discord.Embed(
                    ...
                    title=f"Otherbot settings for {guild.name}",
                )
    ...
    @otherbot.command()
    async def pingrole(
        self, ctx: commands.Context, role_name: discord.Role = None
    ):
    ...
    async def otherbot_watch_offline_add(
        self, ctx: commands.Context, bot: discord.Member
    ):
    ...
    async def otherbot_watch_offline_remove(
        self, ctx: commands.Context, bot: discord.Member
    ):
    ...
    async def otherbot_watch_offline_list(self, ctx: commands.Context):
        ...
        if not watching:
            return await ctx.send(
                "There is currently no bots tracked for offline status."
            )

        watching_list = await self.get_watching(
            watching, "watching", ctx.guild.id
        )
        await ctx.send(
    ...
    async def otherbot_watch_offline_emoji(
        self, ctx: commands.Context, *, emoji: str = None
    ):
        """Choose which emoji that will be used for offline messages."""
        if not emoji:
            await self.config.guild(ctx.guild).offline_emoji.set(
                DEFAULT_OFFLINE_EMOJI
            )
            await ctx.send(
                f"Offline emoji resetted to default: {DEFAULT_OFFLINE_EMOJI}"
            )
        else:
    ...
    async def otherbot_watch_online_add(
        self, ctx: commands.Context, bot: discord.Member
    ):
        ...
        async with self.config.guild(
            ctx.guild
        ).online_watching() as watch_list:
            watch_list.append(bot.id)
        await ctx.send(
            f"I will now track {bot.mention} when it goes back online."
        )
        await self.generate_cache()

    @otherbot_watch_online.command(name="remove")
    async def otherbot_watch_online_remove(
        self, ctx: commands.Context, bot: discord.Member
    ):
        """Removes a bot currently tracked."""
        if not bot.bot:
        ...
        async with self.config.guild(
            ...
            try:
                watch_list.remove(bot.id)
                await ctx.send(
                    f"Successfully removed {bot.mention} from online tracked bot list."
                )
            except ValueError:
                await ctx.send(f"{bot.mention} is not currently tracked.")
        await self.generate_cache()
    ...
    async def otherbot_watch_online_list(self, ctx: commands.Context):
        ...
        if not watching:
            return await ctx.send(
                "There is currently no bots tracked for online status."
            )

        watching_list = await self.get_watching(
            watching, "online_watching", ctx.guild.id
        )
        await ctx.send(
    ...
    async def otherbot_watch_online_emoji(
        self, ctx: commands.Context, *, emoji: str = None
    ):
        """Choose which emoji that will be used for online messages."""
        if not emoji:
            await self.config.guild(ctx.guild).online_emoji.set(
                DEFAULT_ONLINE_EMOJI
            )
            await ctx.send(
                f"Online emoji resetted to default: {DEFAULT_ONLINE_EMOJI}"
            )
        else:
        ...
        if (
            ...
            try:
                if data["embed_offline"]:
                    ...
                    if not data["ping"]:
                        await channel.send(
                            f"{after.mention} is offline. {data['offline_emoji']}"
                        )
                    else:
                        await channel.send(
