...
class LocalPath:
    ...
    _all_music_ext = (
        _FULLY_SUPPORTED_MUSIC_EXT + _PARTIALLY_SUPPORTED_MUSIC_EXT
    )
    ...
    def __init__(self, path, localtrack_folder, **kwargs):
        ...
        self.cwd = Path.cwd()
        _lt_folder = (
            Path(self._localtrack_folder)
            if self._localtrack_folder
            else self.cwd
        )
        _path = Path(path) if path else self.cwd
        if _lt_folder.parts[-1].lower() == "localtracks" and not kwargs.get(
            "forced"
        ):
    ...
    def rglob(self, pattern, folder=False) -> Iterator[str]:
        if folder:
            ...
            return glob.iglob(
                f"{glob.escape(self.path)}{os.sep}**{os.sep}*{pattern}",
                recursive=True,
            )
    ...
    def to_string_user(self, arg: str = None):
        string = str(self.absolute()).replace(
            (str(self.localtrack_folder.absolute()) + os.sep)
            if arg is None
            else arg,
            "",
        )
    ...
    async def tracks_in_tree(self):
        tracks = []
        async for track in self.multirglob(
            *[f"{ext}" for ext in self._all_music_ext]
        ):
            with contextlib.suppress(ValueError):
                if (
                    ...
                    tracks.append(
                        Query.process_input(track, self._localtrack_folder)
                    )
    ...
    async def tracks_in_folder(self):
        tracks = []
        async for track in self.multiglob(
            *[f"{ext}" for ext in self._all_music_ext]
        ):
            with contextlib.suppress(ValueError):
                if (
...
class Query:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.is_apple_music: bool = kwargs.get("apple_music", False)
        ...
        self.apple_music_url: Optional[str] = kwargs.get(
    ...
    def process_input(
        ...
        **kwargs,
        ...
        possible_values.update(
            cls._parse(query, _local_folder_current_path, **kwargs)
        )
        return cls(query, _local_folder_current_path, **possible_values)

    @staticmethod
    def _parse(
        track, _local_folder_current_path: Path, **kwargs
    ) -> MutableMapping:
            ...
            try:
                ...
                if all([query_url.scheme, query_url.netloc, query_url.path]):
                    ...
                    if url_domain in ["youtube.com", "youtu.be"]:
                        ...
                        if "?i=" in track:
                            try:
                                remove_start = track.split("album/")[1]
                            except IndexError:
                                pass
                            try:
                        ...
                        returning["apple_music_url"] = track
