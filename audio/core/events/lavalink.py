...
from redbot.core.i18n import set_contextual_locales_from_guild, Translator
import discord
    ...
    async def lavalink_event_handler(
        ...
        if event_type == lavalink.LavalinkEvents.TRACK_START and notify:
            ...
            if notify_channel and self._has_notify_perms(notify_channel):
                ...
                thumb = None
                if (
                    await self.config.guild(guild).thumbnail()
                ...
                notify_message = await self.send_embed_msg(
                    ...
                    footer=_(
                        "Track length: {length} | Requested by: {user}"
        ...
        if event_type == lavalink.LavalinkEvents.QUEUE_END:
            if not autoplay:
                ...
                if (
                    ...
                    await self.send_embed_msg(
                        notify_channel, title=_("Queue ended.")
                    )
                if disconnect:
                    self.bot.dispatch("red_audio_audio_disconnect", guild)
                    await self.config.guild_from_id(
                ...
                if early_exit:
                    embed = discord.Embed(
                        ...
                        description=_(
                            ...
                            "If this persists, please use the contact "
                            "command to inform the bot owner."
                        ),
                    )
