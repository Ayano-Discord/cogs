...
class PlayerCommands(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def command_play(self, ctx: commands.Context, *, query: str):
        ...
        Either names of songs or links work.
        """
        ...
        if not await self.maybe_charge_requester(
            ctx, guild_data["jukebox_price"]
        ):
            return
        if query.is_spotify:
            return await self._get_spotify_tracks(ctx, query)
        elif query.is_apple_music:
            return await self._get_apple_music_tracks(ctx, query)
        try:
    ...
    @commands.command(name="bumpplay", aliases=["bp", "bplay"])
    @commands.guild_only()
    @commands.bot_has_permissions(embed_links=True)
    ...
    async def command_genre(self, ctx: commands.Context):
        ...
        guild_data = await self.config.guild(ctx.guild).all()
        if guild_data["dj_enabled"] and not await self._can_instaskip(
            ctx, ctx.author
        ):
            return await self.send_embed_msg(
                ctx,
    ...
    async def command_autoplay(self, ctx: commands.Context):
        ...
        if not guild_data["notify"] and not player.fetch(
    ...
    @commands.command(name="playmix", aliases=["pm", "pmix", "mix"])
    @commands.guild_only()
    @commands.bot_has_permissions(embed_links=True)
