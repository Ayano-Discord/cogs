...
class PlayerControllerCommands(MixinMeta, metaclass=CompositeMetaClass):
    @commands.command(name="disconnect", aliases=["dc", "die", "kys"])
    @commands.guild_only()
    ...
    @commands.command(name="prev", aliases=["back", "previous"])
    @commands.guild_only()
    @commands.bot_has_permissions(embed_links=True)
    ...
    @commands.command(name="seek", aliases=["ff", "fastforward", "rewind"])
    @commands.guild_only()
    @commands.bot_has_permissions(embed_links=True)
        ...
        if vote_enabled:
            if not can_skip:
                ...
                if vote >= percent:
                    ...
                    await self.send_embed_msg(
                        ctx, title=_("Vote threshold met.")
                    )
