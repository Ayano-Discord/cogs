        ...
        async with ctx.typing():
            ...
            if playlist is None:
                return await self.send_embed_msg(
                    ...
                    description=_(
                        "Could not match '{arg}' to a playlist"
                    ).format(arg=playlist_arg),
                )
            ...
            to_append = await self.fetch_playlist_tracks(
                ctx,
                player,
                Query.process_input(query, self.local_folder_current_path),
            )
            ...
            if to_append and to_append_count == 1:
                ...
                if to in tracks_obj_list:
                    return await self.send_embed_msg(
                        ...
                        footer=_(
                            "Playlist limit reached: Could not add track."
                        ).format(not_added)
                        if not_added > 0
                        else None,
                    )
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            await self.send_embed_msg(
                ...
                description=_(
                    "{name} (`{id}`) [**{scope}**] playlist deleted."
                ).format(name=playlist.name, id=playlist.id, scope=scope_name),
            )
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            if len(track_list) == len(clean_list):
                return await self.send_embed_msg(
                    ctx, title=_("URL not in playlist.")
                )
            ...
            try:
                ...
                if playlist:
                    return await ctx.invoke(
                        self.command_play, query=playlist.url
                    )
        ...
        async with ctx.typing():
            ...
            try:
                ...
                if added or removed:
                    ...
                    if added:
                        added_text = ""
                        async for i, track in AsyncIter(added).enumerate(
                            start=1
                        ):
                            if len(track.title) > 40:
                                track_title = str(track.title).replace("[", "")
