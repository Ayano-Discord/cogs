...
from ...utils import CacheLevel, has_internal_server, PlaylistScope
from ..abc import MixinMeta
from ..cog_utils import __version__, CompositeMetaClass, PlaylistConverter
...
class AudioSetCommands(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def command_audioset_perms_global_whitelist(
    ...
    async def command_audioset_perms_global_blacklist(
        self, ctx: commands.Context
    ):
    ...
    async def command_audioset_perms_global_blacklist_add(
        ...
        if exists:
            ...
            return await self.send_embed_msg(
                ...
                description=_(
                    "Added `{blacklisted}` to the blacklist."
                ).format(blacklisted=keyword),
            )

    @command_audioset_perms_global_blacklist.command(name="list")
    @commands.bot_has_permissions(add_reactions=True)
    async def command_audioset_perms_global_blacklist_list(
        self, ctx: commands.Context
    ):
        ...
        if not blacklist:
            return await self.send_embed_msg(
                ctx, title=_("Nothing in the blacklist.")
            )
        blacklist.sort()
        text = ""
        total = len(blacklist)
    ...
    async def command_audioset_perms_whitelist_add(
        ...
        async with self.config.guild(
            ctx.guild
        ).url_keyword_whitelist() as whitelist:
            if keyword in whitelist:
                exists = True
            else:
                whitelist.append(keyword)
    ...
    @commands.bot_has_permissions(add_reactions=True)
    async def command_audioset_perms_whitelist_list(
        self, ctx: commands.Context
    ):
        """List all keywords added to the whitelist."""
    ...
    async def command_audioset_perms_whitelist_clear(
        self, ctx: commands.Context
    ):
        """Clear all keywords from the whitelist."""
        whitelist = await self.config.guild(ctx.guild).url_keyword_whitelist()
    ...
    @command_audioset_perms_whitelist.command(
        name="delete", aliases=["del", "remove"]
    )
    ...
    async def command_audioset_perms_blacklist_add(
        ...
        async with self.config.guild(
            ctx.guild
        ).url_keyword_blacklist() as blacklist:
            if keyword in blacklist:
                exists = True
            else:
                blacklist.append(keyword)
    ...
    @commands.bot_has_permissions(add_reactions=True)
    async def command_audioset_perms_blacklist_list(
        self, ctx: commands.Context
    ):
        """List all keywords added to the blacklist."""
    ...
    async def command_audioset_perms_blacklist_clear(
        self, ctx: commands.Context
    ):
        """Clear all keywords added to the blacklist."""
        blacklist = await self.config.guild(ctx.guild).url_keyword_blacklist()
    ...
    @command_audioset_perms_blacklist.command(
        name="delete", aliases=["del", "remove"]
    )
    ...
    async def command_audioset_autoplay_toggle(self, ctx: commands.Context):
        ...
        await self.send_embed_msg(
            ctx, title=_("Setting Changed"), description=msg
        )
    ...
    async def command_audioset_historical_queue(self, ctx: commands.Context):
        ...
        await self.config.guild(ctx.guild).daily_playlists.set(
            not daily_playlists
        )
    ...
    async def command_audioset_dj(self, ctx: commands.Context):
        ...
        if dj_role is None:
            ...
            try:
                ...
                await ctx.invoke(
                    self.command_audioset_role, role_name=pred.result
                )
    ...
    async def command_audioset_emptydisconnect(
        ...
        if seconds == 0:
            ...
            await self.send_embed_msg(
                ...
                title=_("Setting Changed"),
                description=_(
                    "Empty disconnect timer set to {num_seconds}."
                ).format(num_seconds=self.get_time_string(seconds)),
            )
    ...
    @commands.guild_only()
    @commands.mod_or_permissions(administrator=True)
    async def command_audioset_emptypause(
        self, ctx: commands.Context, seconds: int
    ):
        ...
        if seconds == 0:
            ...
            await self.send_embed_msg(
                ctx,
                title=_("Setting Changed"),
                description=_("Empty pause disabled."),
            )
        else:
            enabled = True
            await self.send_embed_msg(
                ...
                title=_("Setting Changed"),
                description=_(
                    "Empty pause timer set to {num_seconds}."
                ).format(num_seconds=self.get_time_string(seconds)),
            )
    ...
    @commands.guild_only()
    @commands.mod_or_permissions(administrator=True)
    async def command_audioset_jukebox(
        ...
        if price == 0:
            ...
            await self.send_embed_msg(
                ctx,
                title=_("Setting Changed"),
                description=_("Jukebox mode disabled."),
            )
        else:
            jukebox = True
            await self.send_embed_msg(
    ...
    async def command_audioset_localpath(
        ...
        if not local_path:
            await self.config.localpath.set(
        ...
        if not temp.exists() or not temp.is_dir():
            return await self.send_embed_msg(
                ...
                description=_(
                    "{local_path} does not seem like a valid path."
        ...
        try:
            try:
                if not (logs.exists() and logs.is_file()):
                    return await ctx.send(
                        _("No logs found in your data folder.")
                    )
            except OSError:
            ...
            if check(logs):
                ...
                if check(zip_name):
                    await ctx.send(
                        _(
                            "Logs are too large, you can find them in {path}"
                        ).format(path=zip_name.absolute())
                    )
                    zip_name = None
                else:
            ...
            await ctx.send(
                _("I need to be able to DM you to send you the logs.")
            )
        finally:
    ...
    @commands.guild_only()
    @commands.mod_or_permissions(administrator=True)
    async def command_audioset_countrycode(
    ...
    @commands.is_owner()
    async def command_audioset_cache(
    ...
    @commands.guild_only()
    @commands.admin_or_permissions(manage_roles=True)
    async def command_audioset_maxvolume(
        self, ctx: commands.Context, max_volume: int
    ):
