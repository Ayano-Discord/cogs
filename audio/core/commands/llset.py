...
class LavalinkSetupCommands(MixinMeta, metaclass=CompositeMetaClass):
        ...
        if java_path is None:
            ...
            if not exc.exists() or not exc.is_file():
                return await self.send_embed_msg(
                    ...
                    title=_("Invalid Environment"),
                    description=_(
                        "`{java_path}` is not a valid executable"
                    ).format(java_path=exc_absolute),
                )
    ...
    async def command_llsetup_password(
        self, ctx: commands.Context, password: str
    ):
    ...
    @command_llsetup.command(name="wsport", aliases=["port"])
    async def command_llsetup_wsport(
        self, ctx: commands.Context, ws_port: int
    ):
    ...
    async def command_llsetup_info(self, ctx: commands.Context):
        ...
        try:
            await self.send_embed_msg(
                ctx.author, description=box(msg, lang="ini")
            )
