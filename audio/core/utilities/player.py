...
class PlayerUtilities(MixinMeta, metaclass=CompositeMetaClass):
        ...
        if skip_to_track is not None and skip_to_track != 1:
            if skip_to_track < 1:
                ...
                await self.send_embed_msg(
                    ctx,
                    title=_(
                        "There are only {queuelen} songs currently queued."
                    ).format(queuelen=len(player.queue)),
                )
                return
            embed = discord.Embed(
            ...
            player.queue = player.queue[
                min(skip_to_track - 1, len(player.queue) - 1) : len(
                    player.queue
                )
            ]
        else:
    ...
    async def _get_apple_music_tracks(
        self, ctx: commands.Context, query: Query, forced: bool = False
    ) -> Union[discord.Message, List[lavalink.Track], lavalink.Track]:
        if ctx.invoked_with in ["play", "genre", "p"]:
            enqueue_tracks = True
        ...
        if query.single_track:
            try:
                res = await self.api_interface.apple_music_query(
                    ctx, "track", query, skip_youtube=True, notifier=None
                )
                if not res:
                    title = _("Nothing found.")
                    embed = discord.Embed(title=title)
                    ...
                    return await self.send_embed_msg(ctx, embed=embed)
            except AppleMusicFetchError as error:
                self.update_player_lock(ctx, False)
                return await self.send_embed_msg(
                    ctx, title=error.message.format(prefix=ctx.prefix)
                )
            ...
            try:
                self.update_player_lock(ctx, True)
                track_list = await self.fetch_apple_music_playlist(
                    ctx,
                    "album" if query.is_album else "playlist",
                    query,
                    enqueue_tracks,
    ...
    async def _enqueue_tracks(
        ...
        if type(query) is not list:
            ...
            if not tracks:
                ...
                if (
                    await self.config.use_external_lavalink()
                    and query.is_local
                ):
                    embed.description = _(
        ...
        if not first_track_only and len(tracks) > 1:
            ...
            if len(tracks) > track_len:
                ...
                ).format(bad_tracks=(len(tracks) - track_len))
            else:
                maxlength_msg = ""
            ...
            embed.set_footer(
                ...
                ).format(num=track_len, maxlength_msg=maxlength_msg)
            )
            ...
            try:
                ...
                if not await self.is_query_allowed(
                    ...
                    return await self.send_embed_msg(
                        ctx,
                        title=_("This track is not allowed in this server."),
                    )
                elif guild_data["maxlength"] > 0:
                    if self.is_track_length_allowed(
                ...
                return await self.send_embed_msg(
                    ctx, title=title, description=desc
                )
            except Exception as e:
                self.update_player_lock(ctx, False)
                raise e
            ...
            if not guild_data["shuffle"] and queue_dur > 0:
                embed.set_footer(
                    text=_(
                        "{time} until track playback: #{position} in queue"
                    ).format(
                        time=queue_total_duration,
                        position=before_queue_length + 1,
                    )
        ...
        try:
            ...
            notifier = Notifier(
                ...
                playlist_msg,
                {
                    ...
                    "lavalink_time": _(
                        "Approximate time remaining: {seconds}"
                    ),
                },
            )
    ...
    async def fetch_apple_music_playlist(
        self,
        ctx: commands.Context,
        stype: str,
        query: Query,
        ...
        try:
            ...
            track_list = await self.api_interface.apple_music_enqueue(
                ctx,
                stype,
                query,
                enqueue=enqueue,
        ...
        if in_channel == 0 and user_channel:
            if (
                ...
                await player.move_to(
                    user_channel,
                    deafen=await self.config.guild_from_id(
                        ctx.guild.id
                    ).auto_deafen(),
                )
                return True
    ...
    def is_track_length_allowed(
        self, track: lavalink.Track, maxlength: int
    ) -> bool:
        if track.is_stream:
            return True
