...
class EqualizerUtilities(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def _eq_check(
        ...
        config_bands = await self.config.custom(
    ...
    async def _eq_interact(
        ...
        try:
            ...
            return
        try:
            (react_emoji, react_user) = await self._get_eq_reaction(
                ctx, message, emoji
            )
        except TypeError:
            return
        ...
        if react_emoji == "\N{LEFTWARDS BLACK ARROW}\N{VARIATION SELECTOR-16}":
            ...
            await self._eq_interact(
                ctx, player, eq, message, max(selected - 1, 0)
            )
        ...
        if (
            ...
            await self.remove_react(message, react_emoji, react_user)
            await self._eq_interact(
                ctx, player, eq, message, min(selected + 1, 14)
            )

        if react_emoji == "\N{UP-POINTING SMALL RED TRIANGLE}":
            await self.remove_react(message, react_emoji, react_user)
            _max = float(
                "{:.2f}".format(min(eq.get_gain(selected) + 0.1, 1.0))
            )
        ...
        if react_emoji == "\N{DOWN-POINTING SMALL RED TRIANGLE}":
            await self.remove_react(message, react_emoji, react_user)
            _min = float(
                "{:.2f}".format(max(eq.get_gain(selected) - 0.1, -0.25))
            )
