...
class BaseWrapper:
    ...
    async def init(self) -> None:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            executor.submit(
                self.database.cursor().execute, LAVALINK_CREATE_TABLE
            )
            executor.submit(
                self.database.cursor().execute, LAVALINK_CREATE_INDEX
            )
            executor.submit(
                self.database.cursor().execute, YOUTUBE_CREATE_TABLE
            )
            executor.submit(
                self.database.cursor().execute, YOUTUBE_CREATE_INDEX
            )
            executor.submit(
                self.database.cursor().execute, SPOTIFY_CREATE_TABLE
            )
            executor.submit(
                self.database.cursor().execute, SPOTIFY_CREATE_INDEX
            )
            ...
            for future in concurrent.futures.as_completed(
                ...
                try:
                    ...
                    debug_exc_log(
                        log,
                        exc,
                        "Failed to completed random fetch from database",
                    )
        if not row:
            return None
        if self.fetch_result is None:
...
class LavalinkTableWrapper(BaseWrapper):
    def __init__(
        ...
        self.fetch_for_global: Optional[
            Callable
        ] = LavalinkCacheFetchForGlobalResult
