...
from .apple_music import AppleMusicWrapper
...
class AudioAPIInterface:
    ...
    def __init__(
        ...
        self.apple_music_api: AppleMusicWrapper = AppleMusicWrapper(
            self.bot, self.config, session, self.cog
        )
    ...
    async def get_random_track_from_db(
        self, tries=0
        ...
        try:
            ...
            track = await self.local_cache_api.lavalink.fetch_random(
                query_data
            )
    ...
    async def run_tasks(
        ...
        async with self._lock:
            if lock_id in self._tasks:
                if IS_DEBUG:
                    log.debug(
                        "Running database writes for %d (%s)",
                        lock_id,
                        lock_author,
                    )
                try:
    ...
    async def fetch_from_spotify_api(
        ...
        if recursive is False:
            (call, params) = self.spotify_api.spotify_format_call(
                query_type, uri
            )
            results = await self.spotify_api.make_get_call(call, params)
        else:
            if isinstance(recursive, str):
                results = await self.spotify_api.make_get_call(
                    recursive, params
                )
            else:
                results = {}
        ...
        while True:
            ...
            if query_type == "track":
                ...
                if tracks_raw:
                    new_tracks = [
                        k["track"] for k in tracks_raw if k.get("track")
                    ]
                    tracks.extend(new_tracks)
        ...
        if query_type == "track" and cache_enabled:
            try:
                (
                    val,
                    last_update,
                ) = await self.local_cache_api.spotify.fetch_one(
    ...
    async def spotify_enqueue(
        ...
        try:
            ...
            async for track_count, track in AsyncIter(
                ...
                if (
                    youtube_api_error and not global_entry
                ) or consecutive_fails >= (20 if global_entry else 10):
                    error_embed = discord.Embed(
                        colour=await ctx.embed_colour(),
                        title=_("Failing to get tracks, skipping remaining."),
                    )
            ...
            if enqueue and tracks_from_spotify:
                if total_tracks > enqueued_tracks:
                    maxlength_msg = _(
                        " {bad_tracks} tracks cannot be queued."
                    ).format(bad_tracks=(total_tracks - enqueued_tracks))
                else:
                    maxlength_msg = ""
    ...
    async def fetch_apple_music_query(
        self,
        ctx: commands.Context,
        query_type: str,
        uri: str,
        ...
        """Return YouTube URLS for the Apple Music URL provided."""
        youtube_urls = []
        tracks = await self.fetch_from_apple_music_api(
            query_type, uri, params=None, notifier=notifier, ctx=ctx
        )
        total_tracks = len(tracks)
        track_count = 0
        ...
        async for track in AsyncIter(tracks):
            if isinstance(track, str):
                ...
                continue
            track_info = await self.apple_music_api.get_apple_music_track_info(
                track, ctx
            )
    ...
    async def fetch_from_apple_music_api(
        self,
        query_type: str,
        uri: str,
        recursive: Union[str, bool] = False,
        ...
        """Gets track info from Apple Music API."""
        if recursive is False:
            params: MutableMapping = {}
            results = await self.apple_music_api.do_stuff(uri, params)
            results = await self.apple_music_api.mirror_spotify_data(results)
        else:
            if isinstance(recursive, str):
                results = await self.apple_music_api.do_stuff(
                    recursive, params
                )
                results = await self.apple_music_api.mirror_spotify_data(
                    results
                )
            else:
                results = {}
        ...
        while True:
            ...
            if notifier:
                await notifier.notify_user(
                    current=track_count, total=total_tracks, key="apple_music"
                )
            try:
                if results.get("next") is not None:
                    results = await self.fetch_from_apple_music_api(
                        query_type,
                        uri,
                        results["next"],
                        params,
                ...
                raise AppleMusicFetchError(
                    _("This doesn't seem to be a valid Apple Music URL.")
                )
        return tracks

    async def apple_music_query(
        self,
        ctx: commands.Context,
        query_type: str,
        uri: str,
        ...
        """
        Queries the Database then falls back to Apple Music and YouTube APIs.
        ...
        current_cache_level = CacheLevel(await self.config.cache_level())
        youtube_urls = []
        urls = await self.fetch_apple_music_query(
            ctx,
            query_type,
            uri,
            notifier,
    ...
    async def apple_music_enqueue(
        ...
        """
        Queries the Database then falls back to Apple Music and YouTube APIs then Enqueued matched
        tracks.
        ...
        try:
            ...
            queue_total_duration = self.cog.format_time(queue_dur)
            before_queue_length = len(player.queue)
            tracks_from_apple_music = await self.fetch_from_apple_music_api(
                query_type, uri, params=None, notifier=notifier
            )
            total_tracks = len(tracks_from_apple_music)
            if total_tracks < 1 and notifier is not None:
                lock(ctx, False)
                embed3 = discord.Embed(
                    colour=await ctx.embed_colour(),
                    title=_(
                        "That doesn't seem to be a valid Apple Music URL."
                    ),
                )
                await notifier.update_embed(embed3)
            ...
            apple_music_cache = CacheLevel.set_spotify().is_subset(
                current_cache_level
            )
            async for track_count, track in AsyncIter(
                tracks_from_apple_music
            ).enumerate(start=1):
                track_info = (
                    await self.apple_music_api.get_apple_music_track_info(
                        track, ctx
                    )
                )
                ...
                if (
                    ...
                    if youtube_api_error:
                        lock(ctx, False)
                        raise AppleMusicFetchError(message=youtube_api_error)
                    break
                if not track_object:
                    consecutive_fails += 1
                    continue
                ...
                if enqueue:
                    ...
                    if not player.current:
                        await player.play()
            if enqueue and tracks_from_apple_music:
                if total_tracks > enqueued_tracks:
                    maxlength_msg = _(
                        "{bad_tracks} tracks cannot be queued."
                    ).format(bad_tracks=(total_tracks - enqueued_tracks))
    ...
    async def fetch_youtube_query(
        ...
        track_url = await self.youtube_api.get_call(track_info, player)
        if (
    ...
    async def fetch_track(
        ...
        if cache_enabled and not forced and not query.is_local:
            try:
                (
                    val,
                    last_updated,
                ) = await self.local_cache_api.lavalink.fetch_one(
                    {"query": query_string}
                )
            except Exception as exc:
                debug_exc_log(
                    log,
                    exc,
                    "Failed to fetch %r from Lavalink table",
                    query_string,
                )
        ...
        if (
            ...
            with contextlib.suppress(Exception):
                global_entry = await self.global_cache_api.get_call(
                    query=query
                )
    ...
    async def autoplay(
        self, player: lavalink.Player, playlist_api: PlaylistWrapper
    ):
        ...
        if not tracks or not getattr(playlist, "tracks", None):
            ...
            if not tracks:
                ...
                (results, called_api) = await self.fetch_track(
                    cast(
                        commands.Context,
                        ctx(player.guild, player.guild, self.cog),
                    ),
                    player,
                    Query.process_input(
        ...
        if tracks:
            ...
            if notify_channel_id:
                await self.config.guild_from_id(
                    ...
                    [notify_channel_id, player.channel.id]
                )
            else:
                await self.config.guild_from_id(
                    guild_id=player.guild.id
    ...
    async def fetch_all_contribute(
        self,
    ) -> List[LavalinkCacheFetchForGlobalResult]:
