...
class GlobalCacheWrapper:
    ...
    async def _get_api_key(
        ...
        id_list = list(
            getattr(
                self.bot,
                "all_owner_ids",
                getattr(self.bot, "_true_owner_ids", self.bot.owner_ids),
            )
        )
    ...
    async def get_call(self, query: Optional[Query] = None) -> dict:
        ...
        try:
            ...
            with contextlib.suppress(
                aiohttp.ContentTypeError, asyncio.TimeoutError
            ):
            ...
            debug_exc_log(
                log, err, "Failed to Get query: %s/%s", api_url, query
            )
        return {}
    ...
    async def post_call(
        self, llresponse: LoadResult, query: Optional[Query]
    ) -> None:
        try:
