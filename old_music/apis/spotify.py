...
class SpotifyWrapper:
    ...
    async def get(
        ...
        async with self.session.request(
            ...
            if r.status != 200:
                log.debug(
                    "Issue making GET request to %r: [%d] %r",
                    url,
                    r.status,
                    data,
                )
    ...
    async def get_country_code(self, ctx: Context = None) -> str:
        return (
            await self.config_cache.country_code.get_context_value(
    ...
    async def post(
        ...
        async with self.session.post(url, data=payload, headers=headers) as r:
            ...
            if r.status != 200:
                log.debug(
                    "Issue making POST request to %r: [%d] %r",
                    url,
                    r.status,
                    data,
                )
    ...
    async def get_playlist_from_category(
        self, category: str, ctx: Context = None
    ):
