...
class AudioAPIInterface:
    ...
    async def spotify_query(
        ...
        """
        current_cache_level = (
            await self.config_cache.local_cache_level.get_global()
        )
    ...
    async def autoplay(
        ...
        if (
            not tracks or not getattr(playlist, "tracks", None)
        ) and cache_enabled:
        ...
        if not tracks:
            ...
            (results, called_api) = await self.fetch_track(
                cast(
                    commands.Context, ctx(player.guild, player.guild, self.cog)
                ),
                player,
                Query.process_input(
