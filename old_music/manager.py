...
LAVALINK_APP_YML: Final[pathlib.Path] = (
    LAVALINK_DOWNLOAD_DIR / "application.yml"
)
    ...
    async with aiohttp.ClientSession(json_serialize=json.dumps) as session:
        async with session.get(LAVALINK_JAR_ENDPOINT) as resp:
            ...
            if stable:
                data = list(
                    filter(
                        lambda d: d["prerelease"] is False
                        and d["draft"] is False,
                        data,
                    )
                )
            data = (
                sorted(data, key=lambda k: k["published_at"], reverse=True)[0]
                or {}
            )
            output = (
                data.get("name"),
...
class ServerManager:
    ...
    def __init__(
        self, host: str, password: str, port: int, cache: SettingCacheManager
    ) -> None:
    ...
    async def start(self, java_path: str) -> None:
        ...
        if (
            jar_url := await self.config_cache.managed_lavalink_meta.get_global_url()
        ) is not None:
            self._jar_name = jar_url
            self._jar_download_url = jar_url
            self._jar_build = (
                await self.config_cache.managed_lavalink_meta.get_global_build()
                or self._jar_build
            )
        else:
            if (
                ...
                with contextlib.suppress(Exception):
                    (
                        ...
                        url,
                        _nothing,
                    ) = await get_latest_lavalink_release()
        ...
        if self._proc is not None:
            if self._proc.returncode is None:
                ...
                raise RuntimeError(
                    "Node manager has already been used - create another one"
                )
        ...
        try:
            ...
            log.warning(
                "Timeout occurred whilst waiting for managed node to be ready"
            )
    ...
    async def _get_jar_args(self) -> List[bytes]:
        ...
        if not java_available:
            raise RuntimeError(
                "You must install Java 11 or 13 for Lavalink to run."
            )
        args = [
            self._java_exc,
    ...
    async def _has_java(self) -> Tuple[bool, Optional[Tuple[int, int]]]:
        ...
        if not java_available:
            ...
            self._java_available = (11, 0) <= version < (12, 0) or (
                13,
                0,
            ) <= version < (14, 0)
        return self._java_available, self._java_version
    ...
    async def process_settings(self):
        ...
        try:
            ...
            raise RuntimeError(
                "The value provided for the setting YAML is incorrect."
            )
    ...
    async def _wait_for_launcher(self) -> None:
        ...
        for i in itertools.cycle(range(50)):
            ...
            if _FAILED_TO_START.search(line):
                if b"Port 2333 was already in use" in line:
                    log.warning(
                        "Unable to start managed node; Port 2333 is already in use."
                    )
                    raise ShouldAutoRecover
                raise RuntimeError(
                    "Managed Lavalink node failed to start: %s",
                    line.decode().strip(),
                )
        ...
        if self._proc.returncode == 1:
            raise RuntimeError(
                "Managed node failed to start: Node exited with error code 1."
            )
        if not ready:
    ...
    async def _download_jar(self) -> None:
        ...
        async with aiohttp.ClientSession(json_serialize=json.dumps) as session:
            async with session.get(self._jar_download_url) as response:
                ...
                shutil.move(
                    path, str(LAVALINK_JAR_FILE), copy_function=shutil.copyfile
                )
