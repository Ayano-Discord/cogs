...
class DpyEvents(MixinMeta, ABC, metaclass=CompositeMetaClass):
    async def cog_before_invoke(self, ctx: commands.Context) -> None:
        ...
        if self.command_audioset_lavalink in [
            ctx.command,
            ctx.command.root_parent,
        ]:
            pass
        elif (
            self.lavalink_connect_task
            and self.lavalink_connect_task.cancelled()
        ):
        ...
        ) or await self.config_cache.bot_config.member_is_admin_or_higher(
            ctx.guild, ctx.author
        )
        guild = ctx.guild
        ...
        if dj_enabled:
            ...
            if not dj_roles:
                ...
                await self.send_embed_msg(
                    ctx, title="No DJ role found. Disabling DJ mode."
                )
    ...
    async def cog_command_error(
        ...
        if isinstance(error, CommandRejected):
            ...
            error, KeyError
        ) and "such player for that guild" in str(error):
            handled = True
            await self.send_embed_msg(
                ctx,
            ...
            error, (TrackEnqueueError, asyncio.exceptions.TimeoutError)
        ):
            handled = True
            await self.send_embed_msg(
                ctx,
            ...
            log.exception(
                "This is not handled in the core Music cog, please report it.",
                exc_info=error,
            )
        if not isinstance(
            error,
    ...
    async def on_voice_state_update(
        ...
        if (
            ...
            and await self.config_cache.auto_deafen.get_context_value(
                channel.guild
            )
        ):
            try:
