...
class LavalinkEvents(MixinMeta, ABC, metaclass=CompositeMetaClass):
    async def lavalink_update_handler(
        ...
        event_type: lavalink.enums.PlayerState,
        extra,
    ):
    ...
    async def lavalink_event_handler(
        ...
        if not guild:
            return
        disconnect = await self.config_cache.disconnect.get_context_value(
            guild
        )
        ...
        if event_type == lavalink.LavalinkEvents.WEBSOCKET_CLOSED:
            ...
            if _error_code in [1000] or not guild:
                ...
                return
            await self._ws_op_codes[guild_id].put(
                (event_channel_id, _error_code)
            )
            try:
        ...
        current_id = self.rgetattr(current_track, "_info", {}).get(
            "identifier"
        )
        ...
        log.debug(
            "Received a new lavalink event for %d: %s: %r",
            guild_id,
            event_type,
            extra,
        )
        ...
        if event_type == lavalink.LavalinkEvents.TRACK_END:
            ...
            self.bot.dispatch(
                "red_audio_track_end", guild, prev_song, prev_requester
            )
            player.store("resume_attempts", 0)
        if event_type == lavalink.LavalinkEvents.QUEUE_END:
            prev_requester = player.fetch("prev_requester")
            self.bot.dispatch(
                "red_audio_queue_end", guild, prev_song, prev_requester
            )
            if guild_id:
            ...
            if player.is_auto_playing or (
                ...
                try:
                    ...
                    if notify_channel and self._has_notify_perms(
                        notify_channel
                    ):
        ...
        if event_type == lavalink.LavalinkEvents.TRACK_START and notify:
            ...
            if notify_channel and self._has_notify_perms(notify_channel):
                ...
                dur = (
                    "LIVE"
                    if current_stream
                    else self.format_time(current_length)
                )
                thumb = None
                if (
                    await self.config_cache.thumbnail.get_context_value(
                        player.guild
                    )
        ...
        if event_type == lavalink.LavalinkEvents.TRACK_START and status:
            (
                ...
                get_single_title,
                playing_servers,
            ) = await self.get_active_player_count()
            await self.update_bot_presence(
                current, get_single_title, playing_servers
            )
        ...
        if event_type == lavalink.LavalinkEvents.QUEUE_END:
            if not autoplay:
                ...
                if (
                    notify_channel
                    and notify
                    and self._has_notify_perms(notify_channel)
                ):
                    await self.send_embed_msg(
                        notify_channel, title="Queue ended."
                    )
                if disconnect:
                    self.bot.dispatch("red_audio_audio_disconnect", guild)
                    await self.config_cache.autoplay.set_currently_in_guild(
        ...
        if event_type in [
            ...
            if early_exit:
                ...
                await self.config.custom(
                    "EQUALIZER", str(guild_id)
                ).eq_bands.set(player.equalizer.get())
                await player.stop()
                await player.disconnect()
                await self.config_cache.autoplay.set_currently_in_guild(guild)
            ...
            if message_channel and self._has_notify_perms(message_channel):
                if early_exit:
                    ...
                    if event_type == lavalink.LavalinkEvents.TRACK_STUCK:
                        embed = discord.Embed(
                            colour=await self.bot.get_embed_color(
                                message_channel
                            ),
                            title="Track Stuck",
                            description=(
    ...
    async def _websocket_closed_handler(
        ...
        try:
            ...
            if event_channel_id != channel_id:
                ...
                return
            has_perm = (
                self.can_join_and_speak(player.channel)
                if player.channel
                else False
            )
            if (
