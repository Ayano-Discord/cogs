...
class EqualizerCommands(MixinMeta, ABC, metaclass=CompositeMetaClass):
    ...
    async def command_equalizer_delete(
        self, ctx: commands.Context, eq_preset: str
    ):
        ...
        async with self.config.custom(
            ...
            try:
                if eq_presets[eq_preset][
                    "author"
                ] != ctx.author.id and not await self._can_instaskip(
                    ctx, ctx.author
                ):
        ...
        await self.send_embed_msg(
            ctx,
            title=(
                "The {preset_name} preset was deleted.".format(
    ...
    async def command_equalizer_list(self, ctx: commands.Context):
        ...
        eq_presets = await self.config.custom(
            "EQUALIZER", ctx.guild.id
        ).eq_presets()
        if not eq_presets.keys():
            return await self.send_embed_msg(
                ctx, title="No saved equalizer presets."
            )
    ...
    async def command_equalizer_load(
        self, ctx: commands.Context, eq_preset: str
    ):
        ...
        message = await ctx.send(
            ...
            embed=discord.Embed(
                colour=await ctx.embed_colour(),
                title=(
                    "The {eq_preset} preset was loaded.".format(
                        eq_preset=eq_preset
                    )
                ),
            ),
    ...
    async def command_equalizer_save(
        ...
        if not eq_preset:
            await self.send_embed_msg(
                ctx, title="Please enter a name for this equalizer preset."
            )
            try:
                ...
                eq_preset = (
                    eq_name_msg.content.split(" ")[0].strip('"').lower()
                )
        ...
        if eq_preset in eq_list:
            eq_exists_msg = await self.send_embed_msg(
                ctx,
                title="Preset name already exists, do you want to replace it?",
            )
            start_adding_reactions(
            ...
            if not pred.result:
                ...
                embed2 = discord.Embed(
                    colour=await ctx.embed_colour(), title="Not saving preset."
                )
                ctx.command.reset_cooldown(ctx)
    ...
    async def command_equalizer_set(
        ...
        async with self.config.custom(
            ...
            eq_data["name"] = player.equalizer.name
        band_name = (
            band_names[band_number] if band_int else band_name_or_position
        )
