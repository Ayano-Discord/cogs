...
class PlayerCommands(MixinMeta, ABC, metaclass=CompositeMetaClass):
    ...
    async def command_play(
        self, ctx: commands.Context, *, queries: MultiLineConverter
    ):
        ...
        for query in queries:
            ...
            restrict = await self.config_cache.url_restrict.get_context_value(
                ctx.guild
            )
            ...
            if (
                await self.config_cache.dj_status.get_context_value(ctx.guild)
                and not can_skip
            ):
                return await self.send_embed_msg(
                    ctx,
            ...
            if not self._player_check(ctx):
                if self.lavalink_connection_aborted:
                    ...
                    return await self.send_embed_msg(
                        ctx, title=msg, description=desc
                    )
                try:
                    if not self.can_join_and_speak(
                        ctx.author.voice.channel
                    ) or self.is_vc_full(ctx.author.voice.channel):
                        return await self.send_embed_msg(
                            ctx,
                            title="Unable To Play Tracks",
                            description=(
            ...
            if (
                ...
                or ctx.author.voice.channel != player.channel
            ) and not can_skip:
                return await self.send_embed_msg(
                    ctx,
                    title="Unable To Play Tracks",
    ...
    async def command_bumpplay(
        ...
        *,
        ...
        if isinstance(tracks, discord.Message):
            ...
            self.update_player_lock(ctx, False)
            title = "Unable To Play Tracks"
            desc = "No tracks found for `{query}`.".format(
                query=query.to_string_user()
            )
        ...
        if (
            not play_now
            and not await self.config_cache.shuffle.get_context_value(
                ctx.guild
            )
            and queue_dur > 0
        ):
    ...
    async def command_genre(self, ctx: commands.Context):
        ...
        try:
            category_list = (
                await self.api_interface.spotify_api.get_categories(ctx=ctx)
            )
        ...
        if not category_list:
            return await self.send_embed_msg(
                ctx, title="No categories found, try again later."
            )
        ...
        async for page_num in AsyncIter(range(1, len_folder_pages + 1)):
            embed = await self._build_genre_search_page(
                ctx, category_list, page_num, "Categories"
            )
        ...
        if not cat_menu_output:
            return await self.send_embed_msg(
                ctx, title="No categories selected, try again later."
            )
        ...
        async for page_num in AsyncIter(range(1, len_folder_pages + 1)):
            embed = await self._build_genre_search_page(
                ...
                playlists_list,
                page_num,
                "Playlists for {friendly_name}".format(
    ...
    async def command_autoplay(self, ctx: commands.Context):
        ...
        if not await self.config_cache.notify.get_context_value(
            ctx.guild
    ...
    async def command_search(self, ctx: commands.Context, *, query: str):
        ...
        async def _search_menu(
            ...
            if message:
                await self._search_button_action(
                    search_ctx, tracks, emoji, page
                )
                with contextlib.suppress(discord.HTTPException):
                    await message.delete()
                return None
        ...
        if not isinstance(query, list):
            ...
            if (
                ...
                if (
                    await self.config_cache.dj_status.get_context_value(
                        ctx.guild
                    )
                    and not can_skip
                ):
                ...
                if (
                    not await self.config_cache.shuffle.get_context_value(
                        ctx.guild
                    )
                    and queue_dur > 0
                ):
        ...
        try:
            async with ctx.typing():
                if query.is_spotify:
                    ...
                    tracks = await self._enqueue_tracks(
                        ctx, query, enqueue=False
                    )
                self.update_player_lock(ctx, False)
                if isinstance(tracks, discord.Message):
                    return
                if not tracks:
                    return await self.send_embed_msg(
                        ctx, title="Couldn't generated a mixlist."
                    )
                single_track = (
                    tracks
                    if isinstance(tracks, lavalink.rest_api.Track)
                ...
                mix = (
                    "https://www.youtube.com/watch?v={id}&list=RD{id}".format(
                        id=_id
                    )
                )
                query = Query.process_input(
                    mix, self.local_folder_current_path
                )
                if not await self.maybe_charge_requester(
                    ctx,
                    await self.config_cache.jukebox_price.get_context_value(
