...
class EffectsCommands(MixinMeta, ABC, metaclass=CompositeMetaClass):
    ...
    @command_effects.command(
        name="karaoke", usage="off OR <level> <mono> <band> <width>"
    )
    ...
    @command_effects.command(
        name="timescale", usage="off OR <speed> <pitch> <rate>"
    )
    async def command_effects_timescale(
        ...
        await player.set_filters(
            ...
            distortion=player.distortion
            if player.distortion.changed
            else None,
            timescale=timescale if timescale.changed else None,
            channel_mix=player.channel_mix
            if player.channel_mix.changed
            else None,
    ...
    @command_effects.command(
        name="tremolo", usage="off OR <frequency> <depth>"
    )
    ...
    @command_effects.command(
        name="vibrato", usage="off OR <frequency> <depth>"
    )
    ...
    async def command_effects_lowpass(
        ...
        await player.set_filters(
            ...
            timescale=None
            if low_pass.changed
            else player.timescale,  # Timescale breaks if it applied with lowpass
            channel_mix=player.channel_mix
            if player.channel_mix.changed
            else None,
            volume=player.volume,
    ...
    @command_effects.command(
        name="reset", aliases=["off", "disable", "clear", "remove"]
    )
    ...
    @command_effects.command(
        name="bassboost", aliases=["baseboost"], usage="[off]"
    )
    async def command_effects_bassboost(
        ...
        if state is False or (
            player.equalizer.changed and player.equalizer == eq
        ):
            eq = player.equalizer
            eq.reset()
    ...
    async def command_effects_piano(
        self, ctx: commands.Context, *, state: OffConverter = True
    ):
    ...
    async def command_effects_metal(
        self, ctx: commands.Context, *, state: OffConverter = True
    ):
    ...
    async def command_effects_vaporwave(
        ...
        if state and (
            all([ts != player.timescale, tm != player.tremolo])
    ...
    async def command_effects_synth(
        self, ctx: commands.Context, *, state: OffConverter = True
    ):
