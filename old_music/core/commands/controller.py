...
class PlayerControllerCommands(MixinMeta, ABC, metaclass=CompositeMetaClass):
    ...
    async def command_pause(self, ctx: commands.Context):
        ...
        if player.current and not player.paused:
            await player.pause()
            return await self.send_embed_msg(
                ctx, title="Track Paused", description=description
            )
        if player.current and player.paused:
            await player.pause(False)
            return await self.send_embed_msg(
                ctx, title="Track Resumed", description=description
            )
        ...
        if player.current:
            if player.current.is_stream:
                return await self.send_embed_msg(
                    ctx,
                    title="Unable To Seek Tracks",
                    description="Can't seek on a stream.",
                )
            try:
                int(seconds)
                abs_position = False
            ...
            if not abs_position:
                ...
                if seek <= 0:
                    await self.send_embed_msg(
                        ctx,
                        title="Moved {num_seconds}s to 00:00:00".format(
                            num_seconds=seconds
                        ),
                    )
                else:
    ...
    async def command_shuffle_bumpped(self, ctx: commands.Context):
        ...
        bumped = await self.config_cache.shuffle_bumped.get_context_value(
            ctx.guild
        )
    ...
    async def command_skip(
        ...
        percent = await self.config_cache.votes_percentage.get_context_value(
            ctx.guild
        )
        if vote >= percent:
    ...
    async def command_volume(self, ctx: commands.Context, vol: int = None):
        ...
        (
            max_volume,
            max_source,
        ) = await self.config_cache.volume.get_max_and_source(
    ...
    async def command_remove(
        ...
        if isinstance(index_or_url, int):
            ...
            await self.send_embed_msg(
                ...
                description="Removed {track} from the queue.".format(
                    track=removed_title
                ),
            )
        else:
    ...
    async def command_bump(self, ctx: commands.Context, index: int):
        ...
        await self.send_embed_msg(
            ctx,
            title="Moved track to the top of the queue.",
            description=description,
        )
