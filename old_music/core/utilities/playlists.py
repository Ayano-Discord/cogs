...
CURRATED_DATA = "https://gist.githubusercontent.com/Drapersniper/cbe10d7053c844f8c69637bb4fd9c5c3/raw/json"
...
class PlaylistUtilities(MixinMeta, ABC, metaclass=CompositeMetaClass):
    async def can_manage_playlist(
        ...
        is_different_user = (
            len({playlist.author, user_to_query.id, ctx.author.id}) != 1
        )
        is_different_guild = (
            True
            if guild_to_query is None
        ...
        if getattr(playlist, "id", 0) == 42069:
            ...
            playlist.scope == PlaylistScope.GUILD.value
            and not is_different_guild
        ):
        ...
        if not has_perms:
            if hasattr(playlist, "name"):
                ...
                playlist.scope
                in [PlaylistScope.GUILD.value, PlaylistScope.USER.value]
            ...
            await self.send_embed_msg(
                ctx, title="No access to playlist.", description=msg
            )
            return False
        return True
        ...
        if match_count > 10:
            ...
            raise TooManyMatches(
                (
                    ...
                    match_count=match_count, original_input=original_input
                )
            )
        elif match_count == 1:
            return (
    ...
    async def _load_v3_playlist(
        ...
        if not track_count:
            msg = (
                "Empty playlist {name} (`{id}`) [**{scope}**] created.".format(
            ...
            msg = (
                "Added {num} tracks from the {playlist_name} playlist.".format(
    ...
    async def _load_v2_playlist(
        ...
        playlist_msg = await self.send_embed_msg(ctx, embed=embed1)
        notifier = Notifier(
            ctx, playlist_msg, {"playlist": "Loading track {num}/{total}..."}
        )
        async for track_count, song_url in AsyncIter(
            ...
            try:
                ...
                continue
            if (track_count % 2 == 0) or (
                track_count == len(uploaded_track_list)
            ):
    ...
    async def _maybe_update_playlist(
        ...
        if getattr(playlist, "id", 0) == 42069:
            ...
            new_tracks = [
                lavalink.Track(data=track) for track in updated_tracks
            ]
    ...
    async def fetch_playlist_tracks(
        ...
        if query.is_spotify:
            try:
                ...
                pass
            tracks = await self._get_spotify_tracks(
                ctx, query, forced=skip_cache
            )
