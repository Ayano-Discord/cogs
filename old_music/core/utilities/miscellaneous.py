...
class MiscellaneousUtilities(MixinMeta, ABC, metaclass=CompositeMetaClass):
        ...
        if jukebox and not await self._can_instaskip(ctx, ctx.author):
            ...
            if can_spend:
                ...
                await self.send_embed_msg(
                    ctx,
                    title="Not enough {currency}".format(
    ...
    async def send_embed_msg(
        ...
        **kwargs,
        ...
        if not no_embed:
            ...
            contents = dict(
                title=title, type=_type, url=url, description=description
            )
    ...
    async def get_lyrics_status(self, ctx: Context) -> bool:
        ...
        prefer_lyrics = (
            await self.config_cache.prefer_lyrics.get_context_value(ctx.guild)
        )
        return prefer_lyrics

    async def data_schema_migration(
        ...
        if from_version == to_version:
            return
        if (
            from_version < 2 <= to_version
        ):  # Migrate playlists over to SQL databases.
            ...
            await self.config.custom(PlaylistScope.GUILD.value).set(
                all_playlist
            )
        ...
        if (
            from_version < 5 <= to_version
        ):  # Migrate managed node toggle to new namespace
            async with self.config.all() as global_data:
                use_external_lavalink = global_data.pop(
        ...
        if (
            from_version < 6 <= to_version
        ):  # Migrate node connection info to new namespace
            async with self.config.all() as global_data:
    ...
    def decode_track(
        self, track: str, decode_errors: str = "ignore"
    ) -> MutableMapping:
        """
