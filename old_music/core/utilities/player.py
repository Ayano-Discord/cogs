...
class PlayerUtilities(MixinMeta, ABC, metaclass=CompositeMetaClass):
    ...
    async def get_active_player_count(
        self,
    ) -> Tuple[Optional[Track], Optional[str], int]:
        ...
        try:
            ...
            if current:
                current = lavalink.Track(
                    self.decode_track(current.track_identifier)
                )
        except IndexError:
            current = None
    ...
    async def _skip_action(
        ...
        if not player.current or (not player.queue and not autoplay):
            ...
            if player.current.is_stream:
                ...
                embed.set_footer(
                    text="Currently livestreaming {track}".format(
        ...
        if skip_to_track is not None and skip_to_track != 1:
            if skip_to_track < 1:
                await self.send_embed_msg(
                    ctx,
                    title="Track number must be equal to or greater than 1.",
                )
                return
                ...
                return
            embed = discord.Embed(
                title="{skip_to_track} Tracks Skipped".format(
    ...
    async def self_deafen(self, player: lavalink.Player) -> None:
        ...
        if not guild_id:
            return
        if not await self.config_cache.auto_deafen.get_context_value(
            player.guild
        ):
            return
    ...
    async def _enqueue_tracks(
        ...
        if type(query) is not list:
            if not await self.is_query_allowed(
                ...
                raise QueryUnauthorized(
                    "{query} is not an allowed query.".format(
        ...
        if not first_track_only and len(tracks) > 1:
            ...
            if len(player.queue) >= max_queue_length:
                return await self.send_embed_msg(
                    ctx, title="Queue size limit reached."
                )
            track_len = 0
            ...
            playlist_name = escape(
                playlist_data.name if playlist_data else "No Title",
                formatting=True,
            )
            embed = discord.Embed(
            ...
            embed = discord.Embed(
                title="Track Enqueued", description=description
            )
            if (
                not await self.config_cache.shuffle.get_context_value(
                    ctx.guild
    ...
    async def maybe_move_player(self, ctx: commands.Context) -> bool:
        ...
        user_channel = (
            None if not ctx.author.voice else ctx.author.voice.channel
        )
