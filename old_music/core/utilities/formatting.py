...
class FormattingUtilities(MixinMeta, ABC, metaclass=CompositeMetaClass):
    ...
    async def _search_button_action(
        ...
        if not self._player_check(ctx):
            ...
            try:
                ...
                return await self.send_embed_msg(
                    ctx, title="Connect to a voice channel first."
                )
            except IndexError:
                return await self.send_embed_msg(
                    ctx,
                    title="Connection to Lavalink has not yet been established.",
                )
            ...
            if search_choice.is_local:
                if (
                    ...
                    return await ctx.invoke(
                        self.command_search, query=search_choice
                    )
                elif (
                    search_choice.local_track_path.exists()
    ...
    async def _build_search_page(
        ...
        if hasattr(tracks[0], "uri") and hasattr(
            tracks[0], "track_identifier"
        ):
            title = "Tracks Found:"
        ...
        embed.set_footer(
            text=(
                "Page {page_num}/{total_pages}" + " | {num_results} {footer}"
            ).format(
                page_num=page_num,
        ...
        if track and getattr(track, "uri", None):
            ...
            if query.is_local or "localtracks/" in track.uri:
                if (
                    ...
                    hasattr(track, "title") and track.title != "Unknown title"
                ):
                    if shorten:
                ...
                string = (
                    f"**[{escape(string, formatting=True)}]({track.uri}) **"
                )
    ...
    def format_playlist_picker_data(
        self, pid, pname, ptracks, pauthor, scope
    ) -> str:
