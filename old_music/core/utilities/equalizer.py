...
class EqualizerUtilities(MixinMeta, ABC, metaclass=CompositeMetaClass):
    ...
    async def _eq_interact(
        ...
        async for react_emoji, react_user in self._get_eq_reaction(
            ctx, message, emoji
        ):
            ...
            if react_emoji == "\N{UP-POINTING SMALL RED TRIANGLE}":
                _max = float(
                    "{:.2f}".format(
                        min(equalizer.get_gain(selected) + 0.1, 1.0)
                    )
                )
            ...
            if react_emoji == "\N{DOWN-POINTING SMALL RED TRIANGLE}":
                _min = float(
                    "{:.2f}".format(
                        max(equalizer.get_gain(selected) - 0.05, -0.25)
                    )
                )
