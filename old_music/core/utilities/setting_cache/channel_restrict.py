...
class ChannelRestrictManager(CacheBase):
    ...
    async def get_context_vc(
        self, guild: discord.Guild
    ) -> Set[discord.VoiceChannel]:
        ret: Set[int]
        gid: int = guild.id
        ...
        return {
            vc for cid in ret if (vc := self.bot.get_channel(cid)) is not None
        }

    async def add_to_vc_whitelist(
        self, guild: discord.Guild, ids: Set[int]
    ) -> None:
        gid: int = guild.id
        ids = ids or set()
        if not isinstance(ids, set) or any(
            not isinstance(s, int) for s in ids
        ):
            raise TypeError("IDs objects must be a set of ints")
        if gid not in self._cached_vc:
            self._cached_vc[gid] = set(
                await self._config.guild_from_id(gid).whitelisted_vc()
            )
        for cid in ids:
    ...
    async def remove_from_vc_whitelist(
        self, guild: discord.Guild, ids: Set[int]
    ) -> None:
        gid: int = guild.id
        ids = ids or set()
    ...
    async def get_context_text(
        ...
        return {
            text
            for cid in ret
            if (text := self.bot.get_channel(cid)) is not None
        }

    async def add_to_text_whitelist(
        self, guild: discord.Guild, ids: Set[int]
    ) -> None:
        gid: int = guild.id
        ids = ids or set()
        ...
        if gid not in self._cached_text:
            self._cached_text[gid] = set(
                await self._config.guild_from_id(gid).whitelisted_text()
            )
        for cid in ids:
    ...
    async def remove_from_text_whitelist(
        self, guild: discord.Guild, ids: Set[int]
    ) -> None:
        gid: int = guild.id
        ids = ids or set()
