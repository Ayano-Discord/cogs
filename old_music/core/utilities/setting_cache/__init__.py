...
class SettingCacheManager:
    ...
    blacklist_whitelist: WhitelistBlacklistManager = cache_factory(
    ...
    daily_global_playlist: DailyGlobalPlaylistManager = cache_factory(
        DailyGlobalPlaylistManager
    )
    persistent_queue: PersistentQueueManager = cache_factory(
        PersistentQueueManager
    )
    votes: VotingManager = cache_factory(VotingManager)
    votes_percentage: VotesPercentageManager = cache_factory(
        VotesPercentageManager
    )
    ...
    empty_pause_timer: EmptyPauseTimerManager = cache_factory(
        EmptyPauseTimerManager
    )
    global_api: GlobalDBManager = cache_factory(GlobalDBManager)
    global_api_timeout: GlobalDBTimeoutManager = cache_factory(
        GlobalDBTimeoutManager
    )
    local_cache_level: LocalCacheLevelManager = cache_factory(
        LocalCacheLevelManager
    )
    country_code: CountryCodeManager = cache_factory(CountryCodeManager)
    repeat: RepeatManager = cache_factory(RepeatManager)
    channel_restrict: ChannelRestrictManager = cache_factory(
        ChannelRestrictManager
    )
    ...
    max_track_length: MaxTrackLengthManager = cache_factory(
        MaxTrackLengthManager
    )
    ...
    use_managed_lavalink: ManagedLavalinkManager = cache_factory(
        ManagedLavalinkManager
    )
    managed_lavalink_server_auto_update: LavalinkAutoUpdateManager = (
        cache_factory(LavalinkAutoUpdateManager)
    )
    managed_lavalink_meta: LavalinkJarMetaManager = cache_factory(
        LavalinkJarMetaManager
    )
    managed_lavalink_yaml: ManagedNodeYamlManager = cache_factory(
        ManagedNodeYamlManager
    )
