...
class AutoPlayManager(CacheBase):
    ...
    async def get_currently_in_guild(
        ...
        if self.enable_cache and gid in self._currently_in_cache:
            ...
            ret = await self._config.guild_from_id(
                gid
    ...
    async def set_guild(
        self, guild: discord.Guild, set_to: Optional[bool]
    ) -> None:
        gid: int = guild.id
    ...
    async def set_currently_in_guild(
        ...
        if set_to is not None:
            await self._config.guild_from_id(
                gid
            ).currently_auto_playing_in.set(set_to)
            self._currently_in_cache[gid] = set_to
        else:
            await self._config.guild_from_id(
                gid
            ).currently_auto_playing_in.clear()
    ...
    async def get_currently_in_context_value(
        self, guild: discord.Guild
    ) -> Tuple[int, int]:
