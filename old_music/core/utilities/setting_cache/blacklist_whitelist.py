...
class WhitelistBlacklistManager(CacheBase):
    ...
    async def get_whitelist(
        ...
        if self.enable_cache and gid in self._cached_whitelist:
            ...
            if gid is not None:
                ret = set(
                    await self._config.guild_from_id(
                        gid
    ...
    async def add_to_whitelist(
        ...
        if not isinstance(strings, set) or any(
        ...
        if gid is None:
            if gid not in self._cached_whitelist:
                self._cached_whitelist[gid] = set(
                    await self._config.url_keyword_whitelist()
                )
    ...
    async def add_to_blacklist(
        ...
        strings = strings or set()
        if not isinstance(strings, set) or any(
            not isinstance(r_or_u, str) for r_or_u in strings
        ):
        ...
        if gid is None:
            if gid not in self._cached_blacklist:
                self._cached_blacklist[gid] = set(
                    await self._config.url_keyword_blacklist()
                )
            for string in strings:
                string = string.lower()
    ...
    async def get_context_value(
        ...
        *,
        ...
        return await self.allowed_by_whitelist_blacklist(
            what=what, guild=guild
        )
