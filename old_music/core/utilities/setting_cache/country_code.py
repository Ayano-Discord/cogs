...
class CountryCodeManager(CacheBase):
    ...
    async def set_user(
        self, user: discord.Member, set_to: Optional[str]
    ) -> None:
        uid: int = user.id
        if set_to is not None:
            ...
            self._cached_user[uid] = self._config.defaults["USER"][
                "country_code"
            ]
    ...
    async def set_guild(
        self, guild: discord.Guild, set_to: Optional[str]
    ) -> None:
        gid: int = guild.id
        if set_to:
    ...
    async def get_context_value(
        ...
        return (
            ...
            or await self.get_guild(guild)
            or await self.get_global()
        )
