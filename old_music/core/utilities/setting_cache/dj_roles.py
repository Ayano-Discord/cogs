...
class DJRoleManager(CacheBase):
    ...
    async def get_allowed_members(
        self, guild: discord.Guild
    ) -> Set[discord.Member]:
        members = {
            member
            for m in self.bot.owner_ids
            if (member := guild.get_member(m))
        }
        if await self.config_cache.dj_status.get_context_value(guild):
            members |= {
                member
                for role in await self.get_guild(guild)
    ...
    async def set_guild(
        ...
        if not isinstance(roles, set) and any(
            not isinstance(r, discord.Role) for r in roles
        ):
            raise TypeError("Roles must be a set of discord.Role objects")
        roles_ids = {
            y.id for r in roles if (y := guild.get_role(r.id)) in guild.roles
        }
    ...
    async def remove_guild(
        ...
        gid: int = guild.id
        if not isinstance(roles, set) or any(
            not isinstance(r, discord.Role) for r in roles
        ):
            raise TypeError("Roles must be a set of discord.Role objects")
        if gid not in self._cached_guild:
            self._cached_guild[gid] = await self._config.guild_from_id(
                gid
            ).dj_roles()
        for role in roles:
            role = role.id
