...
# Dependency Imports
from redbot.core import bank, commands, Config, modlog
...
class Reporter(commands.Cog):
    ...
    async def _report_global_guild(self, ctx: commands.Context, cog: str):
        ...
        for k, v in temp.items():
            for gk, gv in v.items():
                if (
                    not isinstance(gv, (str, bool, int, type(None)))
                    or "token" in gk
                ):
    ...
    async def _report_global_member(self, ctx: commands.Context, cog: str):
        ...
        for gk, gv in temp.items():
            for mk, mv in gv.items():
                for k, v in mv.items():
                    if (
                        not isinstance(v, (str, bool, int, type(None)))
                        or "token" in k
                    ):
    ...
    async def _report_user(
        self, ctx: commands.Context, cog: str, user: ConvertUserAPI
    ):
