...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def connect_command(
        self, ctx, *, channel: t.Optional[discord.VoiceChannel]
    ):
    ...
    async def shuffle_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send(
                "The queue could not be shuffled as it is currently empty."
            )
    ...
    async def playing_command(self, ctx):
        ...
        embed.add_field(
            name="Artist",
            value=player.queue.current_track.author,
            inline=False,
        )
