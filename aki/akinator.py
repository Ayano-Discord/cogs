...
from .functions import button_from_json, get_p, read_file
    ...
    async def callback(self, interaction):
        ...
        if self.label == "End Session":
            await interaction.response.edit_message(
                content="Stopped", embed=None, view=None
            )
            self.view.stop()
            return
        ...
        # They answered
            ...
            self.view.all_questions.append(
                {"question": self.view.question, "answered": self.label}
            )
...
class AkinatorView(discord.ui.View):
    ...
    def __init__(self, question, ctx):
        ...
        self.aki = Akinator()
        self.all_questions = []
        self.emoji_config = CustomEmojis.from_json(
            read_file("/home/ubuntu/mine/aki/emojis.json")
        )

        emojis = self.emoji_config
        buttons = [
            {
                "label": "Yes",
    ...
    async def get_embed(self) -> discord.Embed:
        ...
        if self.aki.step is None:
            aki_step = 0
        else:
            aki_step = dself.aki.step
        embed.set_footer(text=f"Question no: {aki_step+1}")
...
class Aki(commands.Cog):
    ...
    async def aki(self, ctx):
        ...
        game = await self.akinator.start_game(client_session=self.session)
